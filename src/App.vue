...
<!-- 
 ...
 [DONE] 2. При введении валюты в поле инпута должны появляться подсказки (autocomplete), которые дают  соответсвие крипто валюты  в процессе ее написания в инпут (максимум 4 предложения)   
 [DONE] 3. Приbчто не существует данной монеты) изменять бэкграунд данной валюты в красный цвет.
...
6. Создать отдельный компонент для работы части API (Графика).
-->
<script>
          ...
          if (t === this.selectedTicker) {
            ...
            while (
              ...
              this.$refs.graph.clientWidth /
                38 /* заменить это число (возможный метод получения это this.$refs.bar.clientWidth)*/
            ) {
              if (!this.$refs.graph || this.$refs.graph === null) {
                return;
              }
